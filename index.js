...
exports.handler = async (request, context) => {
  ...
  if (
    ...
    request.directive.header.namespace === 'Alexa.PlaybackController'
  ) {
    ...
    if (
      ['ChangeChannel', 'SkipChannels'].includes(request.directive.header.name)
    ) {
      await handleChangeChannel(request, context);
    }
  } else if (request.directive.header.namespace === 'Alexa.Speaker') {
    if (
      ['AdjustVolume', 'SetVolume', 'SetMute'].includes(
  ...
  async function handleReportState(request, context) {
    // Get device state
    const state = await homeAssistant.getDeviceState(
